...
namespace Tidbeat.Controllers
{
    ...
    public class CommentsController : Controller
    {
                ...
                try
                {
                    if (User?.Identity.IsAuthenticated == true)
                    {
                        ...
                        if (commentStored != null) {
                            if (user.Id == commentStored.User.Id || _userManager.IsInRoleAsync(user, "Administrator").Result|| _userManager.IsInRoleAsync(user, "Admin").Result || _userManager.IsInRoleAsync(user, "Moderator").Result) //Add for Roles
                            {
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            var user = await _userManager.GetUserAsync(User);
            if (user.Id == comment.User.Id || _userManager.IsInRoleAsync(user, "Moderator").Result || _userManager.IsInRoleAsync(user, "Administrator").Result || _userManager.IsInRoleAsync(user, "Admin").Result) //Add for Roles
            {
